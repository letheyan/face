			...
			public void actionPerformed(ActionEvent e) {	
				if(loginPanel.checkLoginText()) {
					if(login()) {
						...
						frame.setTitle(Res.string().getGate() + " : 进方向" + " : " + Res.string().getOnline());
					}
				}	
			}
		});
    ...
    protected void processEvent(AWTEvent event) {
		if (event instanceof AccessEvent) {                 // 门禁事件处理
	        ...
	        if(msg.UTC == null || msg.UTC.toString().isEmpty()) {
	        	...
	        	datetime = msg.UTC.toString();
	        	timeTextField.setText(datetime);
//	        	System.out.println("开门时间："+datetime);
	        }

	        String open_state = "";
	        // 开门状态
	        if(msg.bStatus == 1) {
				open_state = Res.string().getSucceed();
//	        	openStatusTextField.setText(Res.string().getSucceed());
	        	openStatusTextField.setText(open_state);

	        } else {
				open_state = Res.string().getFailed();
	        	openStatusTextField.setText(open_state);
	        }
//	        System.out.println("开门状态:"+open_state);
...
//	        System.out.println(msg);
			...
			String carname = "";
	        try {
				carname = new String(msg.szCardName, "GBK").trim();
				cardNameTextField.setText(carname);
//				System.out.println("卡名："+ carname);
...
//				System.out.println("体温:"+ String.valueOf(msg.stuManTemperatureInfo.fCurrentTemperature+"℃"));
//				System.out.println("================================");
			...
			if(carname!=""){
				try {
					...
					c = DriverManager.getConnection("jdbc:sqlite:C:\\Users\\yanhy\\Desktop\\JAVA_IN" +
														"\\src\\main\\java\\com\\netsdk\\demo\\frame\\Gate\\gate.db");
					stmt = c.createStatement();

					String sql = "INSERT INTO InRecode (NAME,NameId,DATETIME) " +
								 "VALUES (" + "'" + carname + "'" +"," + "'" + userid + "'" + "," + "'"+ datetime.replace("/","-")+ "'" + ");";
	//				System.out.println(sql);
					stmt.executeUpdate(sql);

	//				ResultSet rs = stmt.executeQuery( "SELECT * FROM InRecode;" );
