...
from sqlalchemy import create_engine, MetaData, Table, update, select, text, desc
    ...
    for i in in_dict.copy():         # i 为 in_dict 字典的 key。
        ...
        if n:
            ...
            if (out_time - in_time).total_seconds() > 0:
                ...
                # 对已出门的人员的状态进行更新。
                con.execute(update(in_table).where(in_table.c.Name == in_dict.get(i).Name,
                                                   in_table.c.STATE == 0).values(STATE=1))
                con.commit()
                in_dict.pop(i)  # 剔除已出门的人员

    # print("在内部人员：", in_dict)  # 生成最终未出门的人员。
    # 以字典格式返回，如：{'小红': (9, '小红', '331022198610013119', datetime.datetime(2021, 4, 8, 14, 31, 48), 0)}
    return in_dict
